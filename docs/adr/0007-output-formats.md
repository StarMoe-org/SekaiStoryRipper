...
- **其他对象**：typetree JSON；带 GameObject 的 bundle 额外输出整个对象图（`_objects.json`），以及没有 container 路径、只能经对象图引用到的 Texture2D（`_textures/<名字>.<pathId>.png`，例如 SpriteMask 用的 `Square`），供 sse 实现特效 prefab。
- **布局**：`library/<bundleName>/<container 相对路径>`，保留原始文件名，`model3.json` 里的相对引用可以直接解析；每话一个 `episodes/…json` 索引，外加 `ripper.lock.json`。
